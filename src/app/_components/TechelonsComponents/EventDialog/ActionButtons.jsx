...
const ActionButtons = memo(({ event, handleRegister, handleShare, shareSuccess, registrationStatus, isLoading }) => {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-2 sticky bottom-0 bg-background/95 backdrop-blur-sm py-3 -mx-3 -mb-3 sm:-mx-4 sm:-mb-4 md:-mx-5 md:-mb-5 lg:-mx-6 lg:-mb-6 px-3 sm:px-4 md:px-5 lg:px-6 border-t shadow-md transition-all duration-300">
      {isLoading ? (
        <Button
          disabled
          className="flex-1 bg-primary/70 h-9 sm:h-10 md:h-11 text-sm sm:text-base"
          aria-label="Loading registration status"
        >
          <Loader2 className="mr-1.5 sm:mr-2 h-3.5 w-3.5 sm:h-4 sm:w-4 flex-shrink-0 animate-spin" />
